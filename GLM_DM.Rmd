...
```{r include=FALSE}
# meteoTest= read.csv("MeteoTestRes.csv")
# #meteoTest<-renameCol(meteoTest)
# summary(meteoTest)
```
...
C'est le modèle qui a le meilleur AIC et l'un des critère BIC les plus faible.
Sinon la pluspart des modèles sont très proches en terme de critère AIC et BIC.
...
La libraire glmulti et sa méthode associée est difficilement utilsable, de part le coût de calcul.
...
Qui se situe autour de 68%-70%. 
Nous n'avons pas pu utiliser nos données de test pour une validation croisé et une évaluation de performance.
...
Le fichier résultat : MeteoPrevision_All.csv contient les données de test avec des colonnes supplémentaires qui sont les prédictions de tous ces modèles.
